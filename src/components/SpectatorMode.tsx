...
export default function SpectatorMode() {
  const [selectedId, setSelectedId] = useState<string>('vuelo_deseo');
  ...
  const activeCombo = spectatorCombos.find(c => c.id === selectedId) || spectatorCombos[0];
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (currentStepIdx < activeCombo.steps.length - 1) {
        ...
        if (autoLoop) {
          // Move to next combo
          const currentIdx = spectatorCombos.findIndex(c => c.id === selectedId);
          const nextIdx = (currentIdx + 1) % spectatorCombos.length;
          setTimeout(() => {
            handleSelectCombo(spectatorCombos[nextIdx].id);
            setIsPlaying(true);
          }, 2000);
        }
      }
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mt-4 relative z-20">
              {/* Left Side: Protagonista HP bar */}
              <div>
                <div className="flex justify-between items-end text-xs mb-1.5 font-black">
                  <span className="text-[#00E5FF] font-display tracking-wide uppercase">
                    PROTAGONISTA
                  </span>
                ...
                <div className="h-4 bg-zinc-900 border-2 border-[#00E5FF] p-0.5">
                  <motion.div 
              ...
              {/* Right Side: Daniel HP bar */}
              <div>
                <div className="flex justify-between items-end text-xs mb-1.5 font-black">
                  <span className="font-mono text-zinc-500 text-[10px]">
                    {danielHP}% HP
                  </span>
                  <span className="text-[#FF2E2E] font-display tracking-wide uppercase">
                    DANIEL (BOSS)
                  </span>
                </div>
                <div className="h-4 bg-zinc-900 border-2 border-[#FF2E2E] p-0.5 rotate-180">
                  <motion.div 
                    className="h-full bg-gradient-to-r from-red-600 to-[#FF2E2E]"
            ...
            <div className="h-48 border-2 border-zinc-900/60 bg-black/60 my-6 relative flex items-center justify-around overflow-hidden p-4 rounded-none">
              ...
              {/* Fighter A: Protagonist / Regina / Lover / Symbol */}
              <div className="w-1/3 flex flex-col items-center justify-center relative z-10">
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className={`w-14 h-14 rounded-none border-2 flex items-center justify-center font-display text-lg font-black uppercase text-shadow-neon-blue relative shadow-2xl ${
                      selectedId === 'furia_carnal'
                        ? 'bg-red-950/80 border-[#FF2E2E] text-[#FF2E2E]'
                        : selectedId === 'eco_niebla'
                        ? 'bg-yellow-950/80 border-yellow-400 text-yellow-400'
                        : selectedId.startsWith('simbolo_')
                        ? 'bg-slate-900 border-slate-500 text-slate-300'
                        : 'bg-sky-950/80 border-[#00E5FF] text-[#00E5FF]'
                    }`}>
              ...
              <div className="w-1/3 flex flex-col items-center justify-center relative z-10">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="flex flex-col items-center"
                  >
                    <div className={`w-14 h-14 rounded-none border-2 flex items-center justify-center font-display text-lg font-black uppercase text-shadow-neon-pink relative ${
                      isPlaying && activeCombo.steps[currentStepIdx]?.defender === 'daniel' && activeCombo.steps[currentStepIdx]?.hitSound
                    ...
                    <span className="text-[9px] font-arcade text-zinc-500 uppercase mt-1.5">DANIEL</span>
                  </motion.div>
                </AnimatePresence>
              </div>
